...
public abstract class Subscriber extends RunnableSubPub {
    private MessageBroker mbIntance;
    private boolean terminated = false;
    private HashMap<Class<? extends Message>, Callback> Massage_Callback;
    ...
    public Subscriber(String name) {
        super(name);
        mbIntance = MessageBrokerImpl.getInstance();
        Massage_Callback = new HashMap<>();

        subscribeBroadcast(FinalThick.class, call -> {
            terminated = true;
        });
    }
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        mbIntance.subscribeEvent(type, this);
        synchronized (Massage_Callback) {
            Massage_Callback.putIfAbsent(type, callback);
        }
    }
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        mbIntance.subscribeBroadcast(type, this);
        synchronized (Massage_Callback) {
            Massage_Callback.putIfAbsent(type, callback);
        }
    }
    ...
    public final void run() {
        initialize(); //to implement in Q,M,Intellgance,MoneyPanny
        mbIntance.register(this);
        Message Mission = null;
        while (!terminated) {
            try {
                Mission = mbIntance.awaitMessage(this);
                Callback MissionCallback = Massage_Callback.get(Mission.getClass());
                if (MissionCallback != null)
                    MissionCallback.call(Mission);
            } catch (InterruptedException e) {
                terminated = true;
            }
        }
        mbIntance.unregister(this); // will get there only when terminated
    }
}
